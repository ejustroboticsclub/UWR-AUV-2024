...
MODEL_PATH = "../../models/best-models/best.pt"
...
class PipeDetection:
    ...
    def detect(self, image: np.ndarray, color_correct=False):
        """
        Detect the pipe in the image.
        Args:
            image (np.ndarray) - BGR format: The image in which the pipe is to be detected.
    ...
    def draw_bounding_boxes(self, image: np.ndarray, results, conf_threshold: float = CONFIDENCE_THRESHOLD, show_conf=True) -> np.ndarray:
        ...
        for r in results:
            boxes = r.boxes
            for box in boxes:
                # Filter out non-pipe detections
                if int(box.cls) != 0:
                    continue
    ...
    def get_bounding_boxes_and_conf_score(self, results) -> tuple[list[list[int]], list[float]]:
        ...
        Returns:
            ...
            None, None: If no pipe is detected in the image.
        """
        scores = []
        bboxes = []
    ...
    def get_direction_green(self, image: np.ndarray, bounding_boxes: list[list[int]], color_correct: bool = False) -> tuple[int, int]:
        """
        Get the direction of the green pipe based on the bounding boxes.
        Args:
            image (np.ndarray) - BGR format: The original image containing the detected pipes.
        ...
        Returns:
            tuple[int, int]: The direction of the green pipe in the format (x, y).
            None, None: If no green pipe is detected in the image.
        """
        x = None
        y = None
        ...
        for bounding_box in bounding_boxes:
            ...
            pipe_image = image[y1:y2, x1:x2]

            red_mask = self.detect_color(pipe_image, "red", color_correct)
            red_pixels = cv2.countNonZero(red_mask)
            total_pixels = pipe_image.shape[0] * pipe_image.shape[1]
            red_ratio = red_pixels / total_pixels

            yellow_mask = self.detect_color(
                pipe_image, "yellow", color_correct)
    ...
    def get_direction_color(self, image: np.ndarray, bounding_boxes: list[list[int]], color: str, color_correct: bool = False) -> tuple[int, int]:
        """
        Get the direction of the pipe based on the color of the pipe.
        Args:
            image (np.ndarray) - BGR format: The original image containing the detected pipes.
            bounding_boxes (list[list[int]]) - A list of bounding boxes in the format [x1, y1, x2, y2].
            color (str) - The color of the pipe to be filtered. It can be 'red', 'green', or 'yellow'.
            color_correct (bool) - Whether to apply color correction to the image before color detection.
        Returns:
            tuple[int, int]: The direction of the pipe in the format (x, y).
            None, None: If no pipe is detected in the image.
        """
        x = None
        y = None
        ...
        for bounding_box in bounding_boxes:
            x1, y1, x2, y2 = bounding_box
            pipe_image = image[y1:y2, x1:x2]
            pipe_mask = self.detect_color(pipe_image, color, color_correct)
            pipe_pixels = cv2.countNonZero(pipe_mask)
            total_pixels = pipe_image.shape[0] * pipe_image.shape[1]
            pipe_ratio = pipe_pixels / total_pixels
            if pipe_ratio >= RATIO_COLOR_DETECTION_THRESHOLD:
                x = (x1 + x2) // 2
                y = y1
                break
    ...
    def get_direction_highest_confidence(self, bounding_boxes: list[list[int]], scores: list[float]) -> tuple[int, int]:
        """
        Get the direction of the pipe based on the highest confidence score.
        Args:
            bounding_boxes (list[list[int]]) - A list of bounding boxes in the format [x1, y1, x2, y2].
    ...
    def get_direction(self, image: np.ndarray, results, filter_criteria: str, color: str = None, color_correct: bool = False) -> tuple[int, int]:
        """
        Get the direction of the pipe based on the filter criteria.
        Args:
            image (np.ndarray) - BGR format: The original image containing the detected pipes.
            results: The YOLOv8 results object containing bounding boxes and confidence scores.
            filter_criteria (str) - The criteria to filter the pipes. It can be 'highest_confidence', or 'color'.
            color (str) - The color of the pipes to be filtered. It can be 'red', 'green', or 'yellow'.
            color_correct (bool) - Whether to apply color correction to the image before color detection.
        Returns:
            tuple[int, int]: The direction of the pipe in the format (x, y).
            None, None: If no pipe is detected in the image or no color is detected in the pipe.
        """
    ...
    def extract_pipes_from_image(self, image: np.ndarray, results) -> list[np.ndarray]:
        """
        Extract the detected pipes from the YOLOv8 results.
        Args:
            image (np.ndarray) - BGR format: The original image containing the detected pipes.
            results: The YOLOv8 results object containing bounding boxes and confidence scores.
        Returns:
            list[np.ndarray]: A list of cropped images of the detected pipes.
            None: If no pipe is detected in the image.
        """
        pipe_images = []
        for r in results:
            boxes = r.boxes
    ...
    def recognize_pipe_color(self, image: np.ndarray, color_correct: bool = False) -> str:
        """
        Recognize the color of the pipe in the image by calculating the ratio of non-zero pixels.
        Args:
            image (np.ndarray) - BGR format: The cropped image of the pipe.
            color_correct (bool) - Whether to apply color correction to the image before color recognition.
        Returns:
            str: The color of the pipe detected in the image. It can be 'red', 'green', 'yellow'.
        ...
        pipe_color = max(color_ratio_dict, key=color_ratio_dict.get)
        highest_ratio = color_ratio_dict[pipe_color]
...
if __name__ == "__main__":
    ...
    # Initialize the PipeDetection class
    pipe_detector = PipeDetection()

    # Detect the pipe in the image
    results = pipe_detector.detect(image, color_correct)

    # Draw bounding boxes on the image
    image_with_boxes = pipe_detector.draw_bounding_boxes(image, results)

    # Extract the detected pipes from the image
    pipe_images = pipe_detector.extract_pipes_from_image(image, results)

    # Display the pipe images
    if pipe_images is not None:
        for i, pipe_image in enumerate(pipe_images):
            cv2.imshow(f"pipe {i + 1}", pipe_image)

    # Detect the red color of the pipes
    if pipe_images is not None:
        for i, pipe_image in enumerate(pipe_images):
            mask = pipe_detector.detect_color(
                pipe_image, "red", color_correct)
            cv2.imshow("mask"+str(i), mask)

    if pipe_images is not None:
        for i, pipe_image in enumerate(pipe_images):
            # Recognize the color of the pipe
            pipe_color = pipe_detector.recognize_pipe_color(
                pipe_image, color_correct)

            if pipe_color is not None:
                print(f"pipe {i + 1} color: {pipe_color}")
            else:
                print(f"pipe {i + 1} color: Unknown")
    else:
        print("No pipe detected in the image.")

    # Display the image with bounding boxes
    cv2.imshow("Detected pipes", image_with_boxes)
    cv2.waitKey(0)
