...
RATIO_COLOR_DETECTION_THRESHOLD = 0.8
...
class BucketDetection:
    ...
    def recognize_bucket_color(self, image: np.ndarray, color_correct : bool = False) -> str:
        ...
        print(f"Red pixels: {red_pixels}, Green pixels: {green_pixels}, Yellow pixels: {yellow_pixels}")
        ...
        print(f"Red ratio: {red_ratio:.2f}, Green ratio: {green_ratio:.2f}, Yellow ratio: {yellow_ratio:.2f}")

        # Store the color ratios in a dictionary
        color_ratio_dict = {"red": red_ratio, "green": green_ratio, "yellow": yellow_ratio}
    ...
    def detect_color(self, image: np.ndarray, color: str, color_correct : bool = False) -> np.ndarray:
        ...
        if color == "red":
            ...
            raise ValueError("Invalid color specified. Please specify 'red', 'green', or 'yellow'.")
...
if __name__ == "__main__":
    ...
    if bucket_images is not None:
        for i, bucket_image in enumerate(bucket_images):
            mask = bucket_detector.detect_color(bucket_image, "red", color_correct)
    ...
    if bucket_images is not None:
        for i, bucket_image in enumerate(bucket_images):
            ...
            bucket_color = bucket_detector.recognize_bucket_color(bucket_image, color_correct)
